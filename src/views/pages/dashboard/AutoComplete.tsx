...
const AutocompleteComponent = ({ hidden }: Props) => {
  ...
  const fullScreenDialog = useMediaQuery(theme.breakpoints.down('sm'))
    ...
    const {names,status}:{names:Array<SearchName>,status:string} = useSelector((state: any) => state.dashboard.searchNames);
  ...
  const handleOptionClick = (obj: SearchName) => {
    setSearchValue('')
    setOpenDialog(false)
    
    
    
    router.push('/organization/'+obj._id)
          ...
          <Dialog fullWidth open={openDialog} fullScreen={fullScreenDialog} onClose={() => setOpenDialog(false)}>
            <Box sx={{ top: 0, width: '100%', position: 'sticky' }}>
              <Autocomplete
                ...
                onChange={(event, obj) => handleOptionClick(obj as SearchName)}

                noOptionsText={<NoResult value={searchValue} setOpenDialog={setOpenDialog} status={status} />}
                getOptionLabel={(option: SearchName | unknown) => (option as SearchName).name}
                ...
                renderOption={(props, option: SearchName | unknown) => {
                    
                  return searchValue.length ? (
                    <ListItem
                      {...props}
                      key={(option as SearchName).name}
                      className={`suggestion ${props.className}`}
                      onClick={() => handleOptionClick(option as SearchName)}
